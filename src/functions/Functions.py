...
##########################################################################
 ...
 ##########################################################################
    ...
    def init_conexion(self, _host = Inicializar.DB_HOST, _port = Inicializar.DB_PORT, _dbname=Inicializar.DB_DATABASE, _user = Inicializar.DB_USER, _pass = Inicializar.DB_PASS):
        try:
            credenciales = {
            "dbname": _dbname,
            "user": _user,
    ...
    def pyodbc_conn(self, _host = Inicializar.DB_HOST, _port = Inicializar.DB_PORT, _dbname=Inicializar.DB_DATABASE, _user = Inicializar.DB_USER, _pass = Inicializar.DB_PASS):
        ...
        try:
            conn =  pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=' +  _host+';DATABASE='+_dbname+';UID='+_user+';PWD=' + _pass)
    ...
    def search_mail(self, search):
        ...
        mailbox.login(Inicializar.mailuser, Inicializar.mailpassword, initial_folder='INBOX')
        subjects = [msg.subject for msg in mailbox.fetch(AND(subject=search))]
        if not subjects:
         return False
        else:
         return True   
    
    def send_mail(self, mensaje, subject):
        msg = MIMEMultipart() 
        message = mensaje
        # setup the parameters of the message
        password = Inicializar.mailpassword
        msg['From'] = Inicializar.mailuser
        msg['To'] = "cristian.rojas@thomasgreg.com"
        msg['Subject'] = subject
